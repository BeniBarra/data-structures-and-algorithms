...
const toLastNames = people => {
  return people.map( i => `${i.firstName} ${i.lastName}`) };
...
const addPurchases = (arr) => {
  return arr.reduce((accum, value) => accum + value.purchasePrice, 0)
};
...
const countNumberOfElements = (arr) => {
  return arr.reduce((accum, value, idx) => idx + 1, 0)
};
...
const returnNames = (arr) => {
  let answer = []
  arr.reduce((accum, value) => answer.push(value.name),'')
  return answer;
};
