...
function transformToLis(obj){
  return Object.entries(obj).map( i => `<li>${i[0]}: ${i[1]}</li>`)
}
...
const count = (target, input) => {
  let answer = 0;
  input.map( i => answer += i.filter( j => j === target).length);
  return answer;
};
...
const totalSum = (input) => {
  let answer = 0;
  input.map( i => answer += i.reduce(( a, b ) => a += b, 0));
  return answer;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let arr = input.map( i => i.filter( j => j%5 === 0));
  arr = arr.map( i => i.filter( i => typeof i === 'number'));
  return arr.map( i => i.map( i => 2**i) );
};
...
let findMaleAndFemale = (data) => {
  let answer = [];
  data.forEach(i => {
    (i.gender === 'male' || i.gender === 'female') ? answer.push(i.name): '';
  })
  answer = answer.join(' and ')
  return answer
};
...
let findShortest = (data) => {
  let heights = data.map(i => i.height)
  heights.sort((a,b) => a-b)
  let answer = data.filter(i => i.height.includes(heights[0]))
