...
const longestString = (arr) => {
  let answer;
  if (arr.length > 0) {
    answer = arr.reduce( ( a, b ) =>  b.length >= a.length ?  a = b : a = a);
  }
  return arr.indexOf(answer);
  // Thank you to Steven and Ben for help on this
};
...
const standardizePhoneNumbers = (arr) => {
  return arr.map(i => i.replace(/\D/g, ''))
  // Thank you Ben Arno for the help on this one
};
...
const onlyOddChars = (str) => {
  let answer = [];
  if (str !== '') {
    str.split('').filter( (ele, i) => i % 2 === 1? answer.push(ele)  : '');
  } 
  return answer.join('');
};
...
const allHappy = (arr) => {
  ...
  return arr.length ===  arr.filter( i => i.includes(':)')).length;
