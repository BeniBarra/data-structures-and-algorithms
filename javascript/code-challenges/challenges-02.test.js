...
const raisedToTheThird = (arr) => {
  const answer = arr.map(num => Math.pow(num,3));
  return answer;
};
...
const addOne = (arr) => {
 const answer = arr.map( num => num + 1);
 return answer
};
...
const addQuestion = (arr) => {
  const answer = arr.map( word => `${word}?`);
  return answer;
};
...
const forEachTwoToThe = (arr) => {
  let answer = [];
  arr.forEach( num => answer.push(Math.pow(2,num)));
  return answer;
};
...
const mapTwoToThe = (arr) => {
  let answer = arr.map( num => Math.pow(2,num));
  return answer;
};
...
const charCode = (arr) => {
  const answer = arr.map( i => i.charCodeAt(arr));
  return answer;
};
...
//thank you to Ben Arno for help on this one
...
const extractAbilities = (arr) => {
  return arr.map( i => i.ability.name);
};

// Thank you to Ben and Steven for help on this
...
const extractStats = (arr) => {
  ...
  return arr.map( i => {
    return {
      name: i.stat.name,
      total: total = i.effort + i.baseStat
    }
  });
};
