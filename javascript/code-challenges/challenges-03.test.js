...
const containsAnd = (arr) => {
  let result = arr.filter(word => word.includes(`and`));
  return result;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  return arr.filter(ele => forbiddenValues.includes(ele) == false);
};
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  return arr.filter(ability => ability.baseStat > minBaseStat);
};
...
const getStatName = (arr, minBaseStat) => {
  let filtered = arr.filter(ability => ability.baseStat > minBaseStat);
  return filtered.map(ele => ele.stat.name);
};
...
const getCharactersWithoutChildren = (arr) => {
  return arr.filter(ele => ele.children == undefined)
};
...
const evenOddNumericValues = (arr) => {
  let result = [];
  let filtered = arr.filter(ele => typeof(ele) == `number`);
