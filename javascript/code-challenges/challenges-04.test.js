...
const updateAnimal = (arr, callback) => {
  return arr.map(ele => ele = callback(ele));
};
...
const sortByLength = (arr) => {
  return arr.sort((a,b) => a.toLowerCase().length < b.toLowerCase().length ? -1 : 1);
};
...
const sortMeetingsByDay = (arr) => {
  return arr.sort((a,b) => days[a.dayOfWeek] < days[b.dayOfWeek] ? -1 : 1);
};
...
const sortSchedule = (arr) => {
  let firstSort = sortMeetingsByDay(arr);
