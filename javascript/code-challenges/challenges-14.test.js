...
const screenForNames = (arr) => arr.filter( i => /^(Mr|Mrs|Ms|Dr)\.\s[a-zA-Z]+$/.test(i));
...
let biggerThanLuke = (arr) => {
  ...
  for (let i = 1; i < starWarsData.length; i ++) {
    if (starWarsData[0].mass < starWarsData[i].mass) {
      box.push(`${starWarsData[i].name}`);
    }
  }
  return box;
};
